'''
This file contains Professor Josh Kimball's git utility functions.
'''
import logging
import subprocess
import os
...
def check_valid_path(func):
    '''
    This function checks to make sure that the passed in path is valid.

    Arguments:
    func -- The function for a path return;
    '''

    @wraps(func)
    def path_checker(path, *args, **kwargs):
        '''
        This function object checks to make sure the path exists.
...
def is_file_in_repo(path):
    '''
    This checks to see if a file is within the git repo.

    Arguments:
    path -- This is the path to check for file existence in repo
    '''
